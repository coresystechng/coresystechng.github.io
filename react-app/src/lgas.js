// src/data/lgas.js
// Exported LGAS object and tolerant lookup helper.
// NOTE: You can extend / edit the arrays below — keep the same shape.
...
const LGAS = {
    ...
    Benue: [
        "Agatu","Apa","Buruku","Gboko","Guma","Gwer East","Gwer West","Katsina-Ala","Konshisha",
        "Kwande","Logo","Makurdi","Obi","Ogbadibo","Ohimini","Oju","Okpokwu","Oturkpo","Tarka","Ukum","Ushongo","Vandeikya"
    ],
    Borno: [
    ...
    Imo: [
        ...
        "Ikeduru","Isiala Mbano","Isu","Mbaitoli","Ngor Okpala","Njaba","Nkwerre","Nwangele","Obowo","Oguta","Ohaji-Egbema","Okigwe","Orlu","Orsu","Oru East","Oru West","Owerri Municipal","Owerri North","Owerri West","Unuimo"
    ],
...
// tolerant lookup that ignores case, spaces and hyphens and supports aliases
const ALIASES = {
  // common variants that should map to the FCT key in LGAS
  "fct": "FCT",
  "fctabuja": "FCT",
  "abuja": "FCT",
  // add other aliases here as needed, e.g.:
  // "lagos": "Lagos",
  // "rivers": "Rivers",
};
...
export function getLGAs(state) {
  ...
  if (LGAS[raw]) return LGAS[raw];

  // 2) alias lookup (normalized)
  const normRaw = normalizeKey(raw);
  if (ALIASES[normRaw] && LGAS[ALIASES[normRaw]]) {
    return LGAS[ALIASES[normRaw]];
  }

  // 3) tolerant normalized key match (normalize keys)
  const foundKey = Object.keys(LGAS).find(k => normalizeKey(k) === normRaw);
  if (foundKey) return LGAS[foundKey];

  // 4) substring match fallback (e.g. "fct" vs "fctabuja")
  const foundBySubstring = Object.keys(LGAS).find(k => {
    const nk = normalizeKey(k);
    return nk.includes(normRaw) || normRaw.includes(nk);
  });
  if (foundBySubstring) return LGAS[foundBySubstring];
